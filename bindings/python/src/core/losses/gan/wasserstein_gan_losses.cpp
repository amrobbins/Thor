...
void validateWassersteinGANCriticLossArguments(const string& loss_name,
    ...
    if (real_scores.getDimensions().empty()) {
        string error_message = loss_name + ": real_scores must have at least one non-batch dimension";
        throw nb::value_error(error_message.c_str());
    }
...
void validateWassersteinGANGeneratorLossArguments(const string& loss_name,
                                                  ...
                                                  optional<DataType> loss_data_type,
                                                  LossShape reported_loss_shape) {
    if (fake_scores.getDimensions().empty()) {
        string error_message = loss_name + ": fake_scores must have at least one non-batch dimension";
        throw nb::value_error(error_message.c_str());
    }
