...
void setReportedLossShape(MAPE::Builder& builder, LossShape shape) {
...
void bind_mean_absolute_percentage_error(nb::module_ &losses) {
    auto loss = nb::class_<MAPE, Loss>(losses, "MAPE");
           ...
           std::optional<float> loss_weight) {
            ...
            if (nb::isinstance<Tensor>(predictionsObject) && nb::isinstance<Tensor>(labelsObject)) {
                ...
                if (predictions.getDimensions() != labels.getDimensions())
                    throw nb::value_error((loss_name + ": predictions and labels dimensions must match").c_str());
                ...
                throw nb::type_error("MAPE predictions and labels must both be thor.Tensor or both be thor.RaggedTensor.");
            }
