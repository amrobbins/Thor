...
def _net():
    return thor.Network("test_net_gaussian_nll_loss")
...
def _gaussian_nll_reference(predictions: np.ndarray, labels: np.ndarray, variance: np.ndarray, full: bool, eps: float) -> np.ndarray:
    predictions = predictions.astype(np.float32)
    labels = labels.astype(np.float32)
    variance = np.maximum(variance.astype(np.float32), eps)
    diff = predictions - labels
    loss = 0.5 * (np.log(variance) + diff * diff / variance)
    if full:
...
def _run_gaussian_nll_loss_network(
    ...
    n = thor.Network(f"test_net_gaussian_nll_loss_numerical_{shape_name}_{full}")
    ...
    outputs = placed.infer(
        {
            ...
            "variance": _cpu_tensor(variance, dtype),
        }
    )
...
def test_gaussian_nll_loss_constructs_defaults():
    n = _net()
    preds = _tensor_1d(5)
    labels = _tensor_1d(5)
...
def test_gaussian_nll_loss_constructs_with_options_loss_dtype_and_shape():
    n = _net()
...
def test_gaussian_nll_loss_reported_loss_shape_variants_construct(shape):
    n = _net()
    preds = _tensor_1d(3)
    labels = _tensor_1d(3)
...
def test_gaussian_nll_loss_accepts_multidimensional_predictions():
    n = _net()
    preds = thor.Tensor([2, 3, 4], thor.DataType.fp32)
    labels = thor.Tensor([2, 3, 4], thor.DataType.fp32)
    variance = thor.Tensor([2, 3, 4], thor.DataType.fp32)

    loss = thor.losses.distribution.GaussianNLLLoss(
        n, preds, labels, variance, reported_loss_shape=thor.losses.LossShape.raw
    )
...
def test_gaussian_nll_loss_numerical_forward_matches_reference(reported_loss_shape, full):
    eps = 1.0e-5
    ...
    raw_expected = _gaussian_nll_reference(predictions, labels, variance, full, eps)
    expected = _reduce_loss(raw_expected, reported_loss_shape)
    actual = _run_gaussian_nll_loss_network(predictions, labels, variance, full, eps, reported_loss_shape)

    np.testing.assert_allclose(actual, expected, rtol=1e-5, atol=1e-6)



def test_gaussian_nll_loss_save_load_round_trip_serializes_support_layers(tmp_path):
    n = thor.Network("test_net_gaussian_nll_loss_round_trip")
    dtype = thor.DataType.fp32
    ...
    save_dir = tmp_path / "gaussian_nll_model"
    n.save(str(save_dir), overwrite=False)
    loaded = thor.Network("test_net_gaussian_nll_loss_round_trip")
