...
def test_asymmetricpowerloss_r10g_constructs_ragged_raw_and_preserves_partition():
    n = _net()
...
def test_asymmetricpowerloss_r10g_rejects_per_output_and_different_partition():
    n = _net()
    predictions, labels = _r10g_ragged_pair(n)
    with pytest.raises(ValueError, match=r"per_output.*undefined"):
        thor.losses.AsymmetricPowerLoss(n, predictions, labels, level=0.8, exponent=1.5, reported_loss_shape=thor.losses.LossShape.per_output)
    ...
    with pytest.raises(ValueError, match=r"exact same row partition"):
        thor.losses.AsymmetricPowerLoss(n, predictions, different_labels, level=0.8, exponent=1.5)


def test_asymmetricpowerloss_r10g_accepts_dense_per_row_example_weights():
    n = _net()
    predictions, labels = _r10g_ragged_pair(n)
    weights_input = thor.layers.NetworkInput(n, "r10g_weights", [1], thor.DataType.bf16)
    weights = weights_input.get_feature_output()
    loss = thor.losses.AsymmetricPowerLoss(n, predictions, labels, level=0.8, exponent=1.5, example_weights=weights)
    assert loss.example_weights == weights
...
def test_asymmetricpowerloss_r10g_matches_dense_prediction_dtype_contract(dtype, expected_loss_dtype):
    n = _net()
    predictions, labels = _r10g_ragged_pair(n, dtype, thor.DataType.int32)
    loss = thor.losses.AsymmetricPowerLoss(n, predictions, labels, level=0.8, exponent=1.5, reported_loss_shape=thor.losses.LossShape.raw)
