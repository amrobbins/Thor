...
def test_adam_rejects_non_positive_alpha():
    ...
    with pytest.raises(ValueError, match=r"alpha must be > 0"):
        thor.optimizers.Adam(n, alpha=0.0)
    with pytest.raises(ValueError, match=r"alpha must be > 0"):
        thor.optimizers.Adam(n, alpha=-1.0)


def test_adam_rejects_beta1_out_of_range():
    n = _net()
    with pytest.raises(ValueError, match=r"0 <= beta1 < 1"):
        thor.optimizers.Adam(n, beta1=-0.01)
    with pytest.raises(ValueError, match=r"0 <= beta1 < 1"):
        thor.optimizers.Adam(n, beta1=1.0)
    with pytest.raises(ValueError, match=r"0 <= beta1 < 1"):
        thor.optimizers.Adam(n, beta1=1.01)


def test_adam_rejects_beta2_out_of_range():
    n = _net()
    with pytest.raises(ValueError, match=r"0 <= beta2 < 1"):
        thor.optimizers.Adam(n, beta2=-0.01)
    with pytest.raises(ValueError, match=r"0 <= beta2 < 1"):
        thor.optimizers.Adam(n, beta2=1.0)
    with pytest.raises(ValueError, match=r"0 <= beta2 < 1"):
        thor.optimizers.Adam(n, beta2=1.01)


def test_adam_rejects_non_positive_epsilon():
    n = _net()
    with pytest.raises(ValueError, match=r"epsilon must be > 0"):
        thor.optimizers.Adam(n, epsilon=0.0)
    with pytest.raises(ValueError, match=r"epsilon must be > 0"):
        thor.optimizers.Adam(n, epsilon=-1e-7)
...
def test_adam_rejects_wrong_types():
    ...
    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, alpha="1e-3")

    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, beta1="0.9")

    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, beta2="0.999")
...
def test_adam_rejects_wrong_arity_and_kwargs():
    ...
    with pytest.raises(TypeError):
        thor.optimizers.Adam()  # missing network

    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, 1e-3, 0.9, 0.999, 1e-7, 123)  # extra positional

    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, bogus=123)  # wrong kw

    with pytest.raises(TypeError):
        thor.optimizers.Adam(n, alpha=1e-3, extra=123)  # extra kw
...
def test_adam_is_optimizer_subclass_if_exposed():
    ...
    n = _net()
    assert isinstance(thor.optimizers.Adam(n), Optimizer)
