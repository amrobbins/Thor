...
def test_sgd_allows_decay_endpoints():
    n = _net()
    assert isinstance(thor.optimizers.Sgd(n, decay=0.0), thor.optimizers.Sgd)
    assert isinstance(thor.optimizers.Sgd(n, decay=1.0), thor.optimizers.Sgd)
...
def test_sgd_rejects_non_positive_initial_learning_rate():
    ...
    with pytest.raises(ValueError, match=r"initial_learning_rate must be > 0"):
        thor.optimizers.Sgd(n, initial_learning_rate=0.0)

    with pytest.raises(ValueError, match=r"initial_learning_rate must be > 0"):
        thor.optimizers.Sgd(n, initial_learning_rate=-1.0)


def test_sgd_rejects_decay_out_of_range():
    n = _net()
    with pytest.raises(ValueError, match=r"0 <= decay <= 1"):
        thor.optimizers.Sgd(n, decay=-0.01)

    with pytest.raises(ValueError, match=r"0 <= decay <= 1"):
        thor.optimizers.Sgd(n, decay=1.01)


def test_sgd_rejects_negative_momentum():
    n = _net()
    with pytest.raises(ValueError, match=r"momentum must be >= 0"):
        thor.optimizers.Sgd(n, momentum=-0.1)
...
def test_sgd_rejects_wrong_types():
    ...
    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, initial_learning_rate="0.1")

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, decay="0.0")

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, momentum="0.0")

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, nesterov_momentum="false")
...
def test_sgd_rejects_wrong_arity_and_kwargs():
    ...
    with pytest.raises(TypeError):
        thor.optimizers.Sgd()  # missing network

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, 0.1, 0.0, 0.0, False, 123)  # extra positional

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, bogus=123)  # wrong kw

    with pytest.raises(TypeError):
        thor.optimizers.Sgd(n, initial_learning_rate=0.1, extra=123)  # extra kw
...
def test_sgd_is_optimizer_subclass_if_exposed():
    ...
    n = _net()
    assert isinstance(thor.optimizers.Sgd(n), Optimizer)
