...
@pytest.mark.parametrize("inclusive", [True, False])
def test_segmented_minmax_scan_backward_uses_arg_scan_and_flat_scatter_add(op: ScanOp, inclusive: bool):
    x = ex.input("x")
    offsets = ex.input("offsets")
    upstream_name = "__grad_output"
    ...
    x_np = np.array([3.0, 1.0, 1.0, 4.0, 2.0, 2.0, 0.0], dtype=np.float32)
    offsets_np = np.array([0, 3, 3, 7], dtype=np.uint32)
