...
def test_physical_tensor_nested_types_exist():
    ...
    assert hasattr(PT, "Placement")
    assert hasattr(PT, "DeviceType")
    ...
    assert hasattr(thor.physical, "Descriptor")
...
def test_descriptor_ctor_rejects_empty_dims():
    with pytest.raises(ValueError, match="dimensions cannot be empty"):
        thor.physical.Descriptor(thor.DataType.fp16, [])


def test_descriptor_basic_properties_and_helpers():
    D = thor.physical.Descriptor
    ...
    assert desc.get_element_name() == "fp16"
    assert D.element_type_name(thor.DataType.fp16) == "fp16"

    # element size (fp16 should be 2 bytes)
...
def test_physical_tensor_construct_copy_deepcopy_cpu():
    ...
    placement = PT.Placement(DT.cpu, 0)
    desc = thor.physical.Descriptor(thor.DataType.fp16, [8, 8])
...
def test_physical_tensor_nested_aliases_match_top_level_types():
    ...
    assert PT.Placement is thor.physical.Placement
    assert PT.DeviceType is thor.physical.DeviceType
    assert PT.Descriptor is thor.physical.Descriptor
