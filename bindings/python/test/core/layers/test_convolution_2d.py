...
def _net(name: str = "test_net_conv2d"):
    return thor.Network(name)


def _chw_input(n: thor.Network, c: int, h: int, w: int, dtype=thor.DataType.fp16, name: str = "input"):
    ni = thor.layers.NetworkInput(n, name, [c, h, w], dtype)
...
def test_conv2d_constructs_defaults_architecture_parameters_and_output_shape_dtype():
    ...
    conv = thor.layers.Convolution2d(n, x, num_output_channels=16, filter_height=3, filter_width=3)

    assert isinstance(conv, thor.layers.Convolution2d)
    y = conv.get_feature_output()
    assert isinstance(y, thor.Tensor)
    assert y.get_dimensions() == [16, 30, 30]
    ...
    _assert_parameter_shape(arch, "weights", [16, 3, 3, 3])
    _assert_parameter_shape(arch, "biases", [16])


def test_conv2d_explicit_tf32_compute_requires_fp32_storage():
    n = _net("test_conv2d_tf32")
    x = _chw_input(n, 4, 8, 8, thor.DataType.fp32)
    conv = thor.layers.Convolution2d(
        n, x, 8, 3, 3, activation=None, compute_data_type=thor.DataType.tf32)
    assert conv.get_compute_data_type() == thor.DataType.tf32
    assert _only_layer_architecture(n, "convolution_2d")["compute_data_type"] == "tf32"

    n_bad = _net("test_conv2d_tf32_bad_storage")
    x_bad = _chw_input(n_bad, 4, 8, 8, thor.DataType.fp16)
    with pytest.raises(ValueError, match="TF32 compute requires FP32"):
        thor.layers.Convolution2d(
            n_bad, x_bad, 8, 3, 3, activation=None, compute_data_type=thor.DataType.tf32)
...
def test_conv2d_accepts_cudnn_frontend_floating_dtype_footprint_at_api_boundary(dtype_name: str, dtype: thor.DataType):
    n = _net(f"test_net_conv2d_dtype_{dtype_name}")
    x = _chw_input(n, 3, 8, 8, dtype)
...
def test_conv2d_has_bias_controls_architecture_and_parameter_set(has_bias: bool):
    ...
    _assert_parameter_shape(arch, "weights", [4, 3, 3, 3])
    if has_bias:
...
def test_conv2d_accepts_epilogue_expression_and_serializes_it():
    ...
    epilogue_input = thor.layers.Convolution2d.epilogue_input(
    ...
    assert arch["epilogue"]["expected_input_names"] == ["__convolution_2d_epilogue_input"]
...
def test_conv2d_epilogue_input_accepts_default_and_explicit_dtypes(output_dtype, compute_dtype):
    kwargs = {}
...
def test_conv2d_rejects_wrong_types_and_arity():
    ...
    with pytest.raises(TypeError):
        thor.layers.Convolution2d(n, x, 8, 3, 3, has_bias="yes")

    with pytest.raises(TypeError):
        thor.layers.Convolution2d(n, x, 8, 3, 3, activation=123)
