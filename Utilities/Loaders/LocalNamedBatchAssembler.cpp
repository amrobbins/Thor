...
LocalNamedBatchAssembler::LocalNamedBatchAssembler(std::vector<std::shared_ptr<Shard>> shards,
    ...
    if (randomizeExamples) {
        shardSelectionRandomizer = std::make_unique<FullPeriodRandom>(numExamples, false);
        if (seed.has_value()) {
            shardSelectionRandomizer->reseed(mixSeed(seed.value(), 0xfeedbeefull));
        }
    }
...
uint64_t LocalNamedBatchAssembler::nextShardSelection() {
    uint64_t selectedExample = 0;
    if (randomizeExamples) {
        THOR_THROW_IF_FALSE(shardSelectionRandomizer != nullptr);
        selectedExample = shardSelectionRandomizer->getRandomNumber();
    } else {
        selectedExample = sequentialGlobalExampleIndex;
        sequentialGlobalExampleIndex = (sequentialGlobalExampleIndex + 1) % numExamples;
    }

    uint64_t priorExamples = 0;
    for (uint64_t i = 0; i < numExamplesPerShard.size(); ++i) {
        priorExamples += numExamplesPerShard[i];
...
void LocalNamedBatchAssembler::batchAssemblerThread() {
    ...
    while (true) {
        ...
        const uint64_t chosenShard = nextShardSelection();
