...
__global__ void sumScale(half *dest, half *noScaleSource, float *scaleSource, float scale, int numElements) {
    int element = blockIdx.x * 512 + threadIdx.x;
...
#pragma unroll 2
    for (int i = 0; i < 2; ++i) {
        ...
        dest[element] = (half)((float)noScaleSource[element] + scale * scaleSource[element]);
...
__global__ void sumScale(float *dest, float *noScaleSource, float *scaleSource, float scale, int numElements) {
    int element = blockIdx.x * 512 + threadIdx.x;
...
#pragma unroll 2
    for (int i = 0; i < 2; ++i) {
        ...
        dest[element] = noScaleSource[element] + scale * scaleSource[element];
