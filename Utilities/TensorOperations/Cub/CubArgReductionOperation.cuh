...
void launchTiledFixedSegmentArgReductionForIndex(const InputT* input,
    ...
    if constexpr (std::is_same_v<IndexT, uint32_t>) {
        if (geometry.inner_size <= 32) {
            ...
            // for that invariant and therefore retain the single conservative generic fallback.
            if (carried_index_input == nullptr) {
    ...
    // Only exceptional domains reach the conservative generic backend:
    //   * UINT32 wide FP8 rows, whose one-byte row-stride alignment cannot be satisfied by the
    //     eight-warp cooperative fast kernel;
    //   * genuinely UINT64 reduction domains.
    //
    // Keep this fallback deliberately singular (RowLanes=1).  The previous exact-width/grouped/sharded
    // fallback families materially multiplied device code while serving no normal FP16/BF16/FP32 UINT32
    // production path.
    if constexpr (std::is_same_v<IndexT, uint32_t>) {
        if constexpr (sizeof(InputT) >= 2) {
            throw std::logic_error(
                "Normal UINT32 FP16/BF16/FP32 tiled ARG geometry must resolve to an ordained aligned fast path.");
        } else {
            if (carried_index_input != nullptr) {
                throw std::logic_error("Composed ARG stages with carried indices must consume FP32 values.");
            }
    ...
    switch (geometry.path) {
        ...
        case CubReductionPath::ContiguousFixedSegment:
            if (geometry.reduction_size > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max())) {
                throw std::logic_error(
                    "Direct contiguous ARG reduction above UINT32 cannot pass the CUB fixed-segment int-size gate.");
            }
            queried_bytes = 1;
            break;
