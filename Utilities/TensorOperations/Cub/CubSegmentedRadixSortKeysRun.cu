...
void cubDeviceSegmentedRadixSortKeys(const CubDeviceSegmentedRadixSortKeysPlan& plan,
                                     ...
                                     Stream& stream) {
    validateSegmentedSortKeys(keys_in, keys_out, segment_offsets, plan.num_items, plan.num_segments, plan.begin_bit, plan.end_bit);
    if (keys_in.getPlacement() != plan.placement || keys_in.getDataType() != plan.key_dtype ||
        segment_offsets.getDataType() != plan.offset_dtype) {
        throw std::invalid_argument("CUB segmented radix-sort-keys plan is not compatible with the provided tensors.");
    }
    ...
    auto launch_key = [&]<typename KeyT>() -> void {
        auto launch_offset = [&]<typename OffsetT>() -> void {
            if (plan.order == CubSortOrder::Ascending) {
                CUDA_CHECK(cub::DeviceSegmentedRadixSort::SortKeys(temp_storage_ptr,
                                                                   temp_storage_bytes,
                ...
                CUDA_CHECK(cub::DeviceSegmentedRadixSort::SortKeysDescending(temp_storage_ptr,
                                                                             temp_storage_bytes,
                                                                             keys_in.getMemPtr<KeyT>(),
                                                                             keys_out.getMemPtr<KeyT>(),
                                                                             cub_items,
...
void cubDeviceSegmentedRadixSortKeys(const Tensor& temp_storage,
    ...
    CubDeviceSegmentedRadixSortKeysPlan plan =
        prepareCubDeviceSegmentedRadixSortKeys(keys_in, keys_out, segment_offsets, num_items, num_segments, order, begin_bit, end_bit);
    if (temp_storage_bytes < plan.temp_storage_bytes) {
        throw std::invalid_argument("temp_storage_bytes is smaller than the prepared CUB segmented radix-sort-keys requirement.");
    }
    plan.temp_storage_bytes = temp_storage_bytes;
    cubDeviceSegmentedRadixSortKeys(plan, temp_storage, keys_in, keys_out, segment_offsets, stream);
}
