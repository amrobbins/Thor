...
__global__ void adamStep(
    ...
    vHalf2[indexHalf2] = __hadd2(__hmul2(beta2Half2, vHalf2[indexHalf2]), __hmul2(__hsub2(ONE_HALF_2, beta2Half2), gradBuffHalf2));
    weightUpdateHalf2[indexHalf2] =
        __hsub2(weightUpdateHalf2[indexHalf2],
                __hmul2(alphaTHalf2, __h2div(mHalf2[indexHalf2], __hadd2(h2sqrt(vHalf2[indexHalf2]), epsilonHalf2))));
};
...
__global__ void adamStep(
    ...
    v[index] = beta2 * v[index] + (1.0f - beta2) * gradBuff;
    weightUpdate[index] = weightUpdate[index] - alphaT * m[index] / (sqrt(v[index]) + epsilon);
};
