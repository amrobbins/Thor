...
#define A_FRAG_HEIGHT 32
#define B_FRAG_WIDTH 8
...
bool TensorCoreMatrixMultiply::checkPreRequisitesBatch8() {
...
#define SPLIT_K 8
__global__ void tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionABMult16(const half *A,
                                                                                const half *B,
                                                                                half *C,
...
#define SPLIT_K 8
__global__ void tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionAMult16(const half *A,
                                                                               const half *B,
                                                                               half *C,
                                                                               const int32_t A_rows,
                                                                               const int32_t A_cols,
...
#define SPLIT_K 4
__global__ void tensorCoreMatrixMultiplyKernel_Arow64_Bcol8_restrictionAMult16(const half *A,
                                                                               const half *B,
                                                                               half *C,
                                                                               const int32_t A_rows,
                                                                               const int32_t A_cols,
...
#define SPLIT_K 2
__global__ void tensorCoreMatrixMultiplyKernel_Arow128_Bcol8_restrictionAMult16(const half *A,
                                                                                const half *B,
                                                                                half *C,
                                                                                const int32_t A_rows,
                                                                                const int32_t A_cols,
...
#define THIS_KERNEL_IS_SPECIFIC_TO_BFRAGWIDTH_OF_8 true
#define B_FRAGS_PER_ITERATION 2
__global__ void tensorCoreMatrixMultiplyKernel_Arow256_Bcol8_restrictionAMult16(const half *A,
                                                                                const half *B,
                                                                                half *C,
                                                                                const int32_t A_rows,
                                                                                const int32_t A_cols,
                                                                                ...
                                                                                const int32_t ld_B,
                                                                                const int32_t ld_C) {
    constexpr int SHARED_HALVES = B_FRAG_WIDTH * A_FRAG_HEIGHT * HALVES_PER_FLOAT > LDB_SHARED * B_FRAG_HEIGHT * B_FRAGS_PER_ITERATION
                                      ? B_FRAG_WIDTH * A_FRAG_HEIGHT * HALVES_PER_FLOAT
                                      : LDB_SHARED * B_FRAG_HEIGHT * B_FRAGS_PER_ITERATION;
    __shared__ half buffer_shared_half[SHARED_HALVES];
    float *buffer_shared = (float *)buffer_shared_half;
...
#pragma unroll
    while (loadACol < A_cols) {
        // Load B_FRAG_HEIGHT*B_FRAGS_PER_ITERATION rows x B_FRAG_WIDTH cols of B, this will be sent to each warp
        int bufferCol = threadIdx.x % B_FRAG_WIDTH;
        ...
        loadBRow += B_FRAG_HEIGHT * B_FRAGS_PER_ITERATION;
        __syncthreads();
        ...
        if (loadACol < A_cols) {
            // Each warp loads the next (to the right horizontally) A_FRAG_HEIGHT rows x A_FRAG_WIDTH cols of A and multipies with B
            wmma::load_matrix_sync(a_frag, A + loadARow * ld_A + loadACol, ld_A);
            wmma::load_matrix_sync(b_frag, buffer_shared_half + B_FRAG_HEIGHT * LDB_SHARED, LDB_SHARED);
            wmma::mma_sync(acc_frag, a_frag, b_frag, acc_frag);
        }
        loadACol += A_FRAG_WIDTH;
    }
...
#undef B_FRAGS_PER_ITERATION
#undef THIS_KERNEL_IS_SPECIFIC_TO_BFRAGWIDTH_OF_8

#define SPLIT_K 8
__global__ void tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_noRestriction(const half *A,
                                                                          const half *B,
                                                                          half *C,
                                                                          const int32_t A_rows,
                                                                          const int32_t A_cols,
    ...
    __shared__ half buffer_shared[SHARED_HALVES];
    float *buffer_shared_float = (float *)buffer_shared;
...
#define SPLIT_K 4
__global__ void tensorCoreMatrixMultiplyKernel_Arow64_Bcol8_noRestriction(const half *A,
                                                                          const half *B,
                                                                          half *C,
                                                                          const int32_t A_rows,
                                                                          const int32_t A_cols,
...
#define SPLIT_K 2
__global__ void tensorCoreMatrixMultiplyKernel_Arow128_Bcol8_noRestriction(const half *A,
                                                                           const half *B,
                                                                           half *C,
                                                                           const int32_t A_rows,
                                                                           const int32_t A_cols,
...
__global__ void tensorCoreMatrixMultiplyKernel_Arow256_Bcol8_noRestriction(const half *A,
                                                                           const half *B,
                                                                           half *C,
                                                                           const int32_t A_rows,
                                                                           const int32_t A_cols,
...
void launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionABMult16(const half *A,
                                                                           const half *B,
                                                                           half *C,
                                                                           half *workspace,
                                                                           const int32_t A_rows,
                                                                           ...
                                                                           Stream stream) {
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 31) / 32);
    tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionABMult16<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionAMult16(const half *A,
                                                                          const half *B,
                                                                          half *C,
                                                                          half *workspace,
                                                                          const int32_t A_rows,
    ...
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 31) / 32);
    tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionAMult16<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow64_Bcol8_restrictionAMult16(const half *A,
                                                                          const half *B,
                                                                          half *C,
                                                                          half *workspace,
                                                                          const int32_t A_rows,
                                                                          ...
                                                                          Stream stream) {
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 63) / 64);
    tensorCoreMatrixMultiplyKernel_Arow64_Bcol8_restrictionAMult16<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow128_Bcol8_restrictionAMult16(const half *A,
                                                                           const half *B,
                                                                           half *C,
                                                                           half *workspace,
                                                                           const int32_t A_rows,
                                                                           ...
                                                                           Stream stream) {
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 127) / 128);
    tensorCoreMatrixMultiplyKernel_Arow128_Bcol8_restrictionAMult16<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow256_Bcol8_restrictionAMult16(const half *A,
                                                                           const half *B,
                                                                           half *C,
                                                                           half *workspace,
                                                                           const int32_t A_rows,
                                                                           ...
                                                                           Stream stream) {
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 255) / 256);
    tensorCoreMatrixMultiplyKernel_Arow256_Bcol8_restrictionAMult16<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_noRestriction(const half *A,
                                                                     const half *B,
                                                                     half *C,
                                                                     half *workspace,
                                                                     const int32_t A_rows,
    ...
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 31) / 32);
    tensorCoreMatrixMultiplyKernel_Arow32_Bcol8_noRestriction<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow64_Bcol8_noRestriction(const half *A,
                                                                     const half *B,
                                                                     half *C,
                                                                     half *workspace,
                                                                     const int32_t A_rows,
    ...
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 63) / 64);
    tensorCoreMatrixMultiplyKernel_Arow64_Bcol8_noRestriction<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow128_Bcol8_noRestriction(const half *A,
                                                                      const half *B,
                                                                      half *C,
                                                                      half *workspace,
                                                                      const int32_t A_rows,
    ...
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 127) / 128);
    tensorCoreMatrixMultiplyKernel_Arow128_Bcol8_noRestriction<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

void launchTensorCoreMatrixMultiplyKernel_Arow256_Bcol8_noRestriction(const half *A,
                                                                      const half *B,
                                                                      half *C,
                                                                      half *workspace,
                                                                      const int32_t A_rows,
    ...
    dim3 blockSize(32, 8);
    dim3 gridSize((B_cols + 7) / 8, (A_rows + 255) / 256);
    tensorCoreMatrixMultiplyKernel_Arow256_Bcol8_noRestriction<<<gridSize, blockSize, 0, stream.getStream()>>>(
        A, B, C, A_rows, A_cols, B_cols, ld_A, ld_B, ld_C);
}

vector<KernelWithSpec> TensorCoreMatrixMultiply::getBCol8Kernels() {
    vector<KernelWithSpec> kernels;
    KernelWithSpec kernel;

    kernel.aRowsPerBlock = 32;
    kernel.bColsPerBlock = 8;
    kernel.aRowSizeModulusRequirement = 32;
    kernel.aColSizeModulusRequirement = 16;
    kernel.bRowSizeModulusRequirement = 16;
    kernel.bColSizeModulusRequirement = 8;
    kernel.id = KernelWithSpec::KernelIndex::_32_8_ABRestrict;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionABMult16;
    ...
    kernel.id = KernelWithSpec::KernelIndex::_32_8_ARestrict;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_restrictionAMult16;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_64_8_ARestrict;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow64_Bcol8_restrictionAMult16;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_128_8_ARestrict;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow128_Bcol8_restrictionAMult16;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_256_8_ARestrict;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow256_Bcol8_restrictionAMult16;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_32_8_loadToShared;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow32_Bcol8_noRestriction;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_64_8_loadToShared;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow64_Bcol8_noRestriction;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_128_8_loadToShared;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow128_Bcol8_noRestriction;
    kernels.push_back(kernel);
    ...
    kernel.bRowSizeModulusRequirement = 0;
    kernel.bColSizeModulusRequirement = 0;
    kernel.id = KernelWithSpec::KernelIndex::_256_8_loadToShared;
    kernel.executeKernel = launchTensorCoreMatrixMultiplyKernel_Arow256_Bcol8_noRestriction;
    kernels.push_back(kernel);
