...
//                                         int num_threads) {
// #ifndef _OPENMP
//     std::printf("%s: OpenMP not enabled; skipping MT benchmark.\n", name);
//     return 0.0;
// #else
//     // Build per-thread buffers deterministically (different seed per thread)
//     std::vector<std::vector<uint8_t> > bufs(num_threads);
//     for (int t = 0; t < num_threads; ++t) {
//         bufs[t].resize(per_thread_bytes);
//         std::mt19937_64 rng(123 + 1000ull * (uint64_t)t);
//         for (auto& b : bufs[t])
...
//
//     std::printf(
//         "%s (OpenMP %d threads, %.2f MiB/thread): %.2f GB/s\n", name, num_threads, double(per_thread_bytes) / (1024.0 * 1024.0), gbs);
//     return gbs;
// #endif
// }
//
// static uint32_t crc32c_sse42_wrap(const uint32_t crc_accum, const uint8_t* data, const size_t len) {
//     // assuming your API is update(crc, data, len)
//     return thor_file::Crc32c::update(crc_accum, data, len);
//     // extern uint32_t crc32c_update(uint32_t crc, const void* data, size_t len);  // or include your header
//     // return crc32c_update(0u, data, len);
// }
//
// static double bench_memcpy_gbs(const char* name, std::vector<uint8_t>& dst, const std::vector<uint8_t>& src, int iters) {
//     if (dst.size() != src.size()) {
//         std::printf("%s: dst/src size mismatch\n", name);
//         return 0.0;
//     }
//
//     // warmup
...
//     (void)sink;
//
//     std::printf("%s: %.2f GB/s\n", name, gbs);
//     return gbs;
// }
//
// static double bench_memread_gbs(const char* name, const std::vector<uint8_t>& buf, int iters) {
...
//     return 0.0;
// #else
//     std::vector<std::vector<uint8_t> > src(num_threads), dst(num_threads);
//     for (int t = 0; t < num_threads; ++t) {
...
//     const double gbs = bytes / sec / 1e9;
//
//     std::printf("%s (OpenMP %d threads, %.2f MiB/thread): %.2f GB/s (sink=%llx)\n",
//                 name,
//                 num_threads,
...
//
//     // 1-thread baselines
//     bench_gbs("CRC32C (SSE4.2 _mm_crc32_u64)", &crc32c_sse42_wrap, buf, iters);
...
//
//     bench_gbs_omp_independent("CRC32C (SSE4.2 _mm_crc32_u64)", &crc32c_sse42_wrap, sz, mt_iters, threads);
