...
class AdamW : public Optimizer {
   public:
    ...
    std::string getType() const override { return "AdamW"; }
...
class AdamW::Builder {
   public:
    virtual std::shared_ptr<AdamW> build() {
        if (!_alpha.has_value())
            _alpha = 0.001f;
        if (!_beta1.has_value())
            _beta1 = 0.9f;
        ...
        AdamW adamw;
        adamw.alpha = _alpha.value();
        adamw.beta1 = _beta1.value();
        adamw.beta2 = _beta2.value();
        adamw.epsilon = _epsilon.value();
        adamw.weightDecay = _weightDecay.value();

        THOR_THROW_IF_FALSE(adamw.alpha > 0.0f);
        THOR_THROW_IF_FALSE(adamw.beta1 >= 0.0f && adamw.beta1 < 1.0f);
        THOR_THROW_IF_FALSE(adamw.beta2 >= 0.0f && adamw.beta2 < 1.0f);
        THOR_THROW_IF_FALSE(adamw.epsilon > 0.0f);
        THOR_THROW_IF_FALSE(adamw.weightDecay >= 0.0f);
        ...
        if (_network.has_value() && _network.value() != nullptr) {
            adamw.addToNetwork(_network.value());
            THOR_THROW_IF_FALSE(std::dynamic_pointer_cast<AdamW>(_network.value()->getDefaultOptimizer()) != nullptr);
            return std::dynamic_pointer_cast<AdamW>(_network.value()->getDefaultOptimizer());
        } else {
            return std::dynamic_pointer_cast<AdamW>(adamw.clone());
        }
    }
    virtual AdamW::Builder &network(Network &_network) {
        THOR_THROW_IF_FALSE(!this->_network.has_value());
        this->_network = &_network;
        return *this;
    }
    virtual AdamW::Builder &alpha(float _alpha) {
        THOR_THROW_IF_FALSE(!this->_alpha.has_value());
        this->_alpha = _alpha;
        return *this;
    }
    virtual AdamW::Builder &beta1(float _beta1) {
        THOR_THROW_IF_FALSE(!this->_beta1.has_value());
        this->_beta1 = _beta1;
        return *this;
    }
    virtual AdamW::Builder &beta2(float _beta2) {
        THOR_THROW_IF_FALSE(!this->_beta2.has_value());
        this->_beta2 = _beta2;
        return *this;
    }
    virtual AdamW::Builder &epsilon(float _epsilon) {
        THOR_THROW_IF_FALSE(!this->_epsilon.has_value());
        this->_epsilon = _epsilon;
        return *this;
    }
    virtual AdamW::Builder &weightDecay(float _weightDecay) {
