...
void validateSourceStorage(const DatasetLayout::WindowedTensorSpec &spec) {
    ...
    if (!spec.sourceFilename.has_value() || spec.sourceFilename->empty()) {
        throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source storage is missing source file.");
    }
    ...
    for (const DatasetLayout::WindowedTensorSourceSequence &sequence : spec.sourceSequences) {
        if (sequence.keyHex.empty()) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source sequence has empty key_hex.");
        }
        if (!keys.insert(sequence.keyHex).second) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' has duplicate source key.");
        }
        if (sequence.numSteps == 0) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source sequence has zero steps.");
        }
        if (sequence.endIndexExclusive <= sequence.startIndex) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source sequence has invalid index bounds.");
        }
        ...
        if (sequence.numSteps != expectedSteps) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source sequence num_steps does not match bounds.");
        }
        ...
        if (sequence.numBytes != expectedBytes) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source sequence num_bytes does not match step shape.");
        }
    ...
    if (maxEndBytes != spec.sourceNumBytes) {
        throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' source bytes do not match source sequences.");
    }
}
...
DatasetLayout::TensorShape::TensorShape(std::string name,
...
uint64_t DatasetLayout::WindowedTensorSpec::outputNumBytes() const { return expectedNumBytes(dimensions, dataType, name); }

uint64_t DatasetLayout::WindowedTensorSpec::keyNumBytes() const { return checkedWholeByteElementSizeBytes(keyDataType); }

uint64_t DatasetLayout::WindowedTensorSpec::indexNumBytes() const { return checkedWholeByteElementSizeBytes(indexDataType); }
...
const DatasetLayout::TensorSpec &DatasetLayout::tensor(std::string_view name) const {
    ...
    if (it == layoutTensors.end()) {
        throw std::runtime_error("DatasetLayout tensor not found: " + std::string(name));
    }
    return *it;
}
...
const DatasetLayout::WindowedTensorSpec &DatasetLayout::windowedTensor(std::string_view name) const {
    ...
    if (it == layoutWindowedTensors.end()) {
        throw std::runtime_error("DatasetLayout windowed tensor not found: " + std::string(name));
    }
    return *it;
}

const std::vector<DatasetLayout::WindowedTensorSpec> &DatasetLayout::windowedTensors() const {
    return layoutWindowedTensors;
}

void DatasetLayout::validate() const {
    if (layoutRecordSizeBytes == 0) {
        throw std::runtime_error("DatasetLayout record_size_bytes must be non-zero.");
    }
    ...
    for (const TensorSpec &spec : layoutTensors) {
        if (spec.name.empty()) {
            throw std::runtime_error("DatasetLayout tensor names must be non-empty.");
        }
        if (!names.insert(spec.name).second) {
            throw std::runtime_error("DatasetLayout duplicate tensor name: " + spec.name);
        }
        ...
        if (end > layoutRecordSizeBytes) {
            throw std::runtime_error("DatasetLayout tensor '" + spec.name + "' extends past record_size_bytes.");
        }
    ...
    for (const WindowedTensorSpec &spec : layoutWindowedTensors) {
        if (spec.name.empty()) {
            throw std::runtime_error("DatasetLayout windowed tensor names must be non-empty.");
        }
        if (!names.insert(spec.name).second) {
            throw std::runtime_error("DatasetLayout duplicate tensor/windowed tensor name: " + spec.name);
        }
        if (!isIntegerDataType(spec.keyDataType)) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' key dtype must be an integer type.");
        }
        if (!isIntegerDataType(spec.indexDataType)) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' index dtype must be an integer type.");
        }
        ...
        if (spec.referenceNumBytes != expectedReferenceBytes) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' reference_num_bytes " +
        ...
        if (referenceEnd > layoutRecordSizeBytes) {
            throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' reference extends past record_size_bytes.");
        }
        if (spec.maskName.has_value()) {
            if (spec.maskName->empty()) {
                throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' mask name must be non-empty.");
            }
            if (names.find(*spec.maskName) != names.end()) {
                throw std::runtime_error("DatasetLayout windowed tensor '" + spec.name + "' mask name collides with tensor name: " +
                                         *spec.maskName);
            }
            if (!maskNames.insert(*spec.maskName).second) {
                throw std::runtime_error("DatasetLayout duplicate windowed tensor mask name: " + *spec.maskName);
            }
        }
...
void DatasetLayout::validateRequestedLayoutExact(const DatasetLayout &requested) const {
    ...
    if (layoutRecordSizeBytes != requested.layoutRecordSizeBytes) {
        throw std::runtime_error("DatasetLayout requested record_size_bytes " + std::to_string(requested.layoutRecordSizeBytes) +
    ...
    if (layoutTensors.size() != requested.layoutTensors.size()) {
        throw std::runtime_error("DatasetLayout requested tensor count " + std::to_string(requested.layoutTensors.size()) +
    ...
    if (layoutWindowedTensors.size() != requested.layoutWindowedTensors.size()) {
        throw std::runtime_error("DatasetLayout requested windowed tensor count " +
    ...
    for (const TensorSpec &manifestSpec : layoutTensors) {
        ...
        if (manifestSpec != requestedSpec) {
            ...
            message << "DatasetLayout requested tensor '" << manifestSpec.name << "' does not match manifest.";
    ...
    for (const WindowedTensorSpec &manifestSpec : layoutWindowedTensors) {
        ...
        if (!manifestSpec.contractEquals(requestedSpec)) {
            std::ostringstream message;
            message << "DatasetLayout requested windowed tensor '" << manifestSpec.name << "' does not match manifest.";
            if (manifestSpec.dimensions != requestedSpec.dimensions) {
                message << " shape manifest=" << shapeToString(manifestSpec.dimensions)
                        << " requested=" << shapeToString(requestedSpec.dimensions) << '.';
            }
...
json DatasetLayout::toJson() const {
    ...
    for (const TensorSpec *spec : sortedSpecs) {
        root["tensors"][spec->name] = json{{"shape", spec->dimensions},
                                                  {"data_type", dataTypeToString(spec->dataType)},
                                                  {"offset_bytes", spec->offsetBytes},
                                                  {"num_bytes", spec->numBytes}};
    }
...
DatasetLayout DatasetLayout::fromJson(const json &j) {
    if (!j.is_object()) {
        throw std::runtime_error("DatasetLayout manifest must be a JSON object.");
    }
    ...
    if (!tensorJson.is_object()) {
        throw std::runtime_error("DatasetLayout manifest tensors field must be an object.");
    }
    ...
    if (j.contains("windowed_tensors")) {
        ...
        if (!windowedTensorJson.is_object()) {
            throw std::runtime_error("DatasetLayout manifest windowed_tensors field must be an object.");
        }
        ...
        for (const auto &item : windowedTensorJson.items()) {
            ...
            if (specJson.contains("source_storage")) {
                ...
                if (!sequencesJson.is_array()) {
                    throw std::runtime_error("DatasetLayout windowed tensor '" + name + "' source sequences field must be an array.");
                }
    ...
    return DatasetLayout(recordSizeBytes, std::move(specs), std::move(windowedSpecs));
}

void DatasetLayout::writeManifest(const std::filesystem::path &path) const {
    std::ofstream out(path, std::ios::binary | std::ios::trunc);
    if (!out.is_open()) {
        throw std::runtime_error("DatasetLayout failed to open manifest for writing: " + path.string());
    }
    out << toJson().dump(2) << '\n';
    if (!out.good()) {
        throw std::runtime_error("DatasetLayout failed while writing manifest: " + path.string());
    }
}
...
DatasetLayout DatasetLayout::readManifest(const std::filesystem::path &path) {
    ...
    if (!in.is_open()) {
        throw std::runtime_error("DatasetLayout failed to open manifest for reading: " + path.string());
    }
    json j;
    in >> j;
    if (!in.good() && !in.eof()) {
        throw std::runtime_error("DatasetLayout failed while reading manifest: " + path.string());
    }
    return fromJson(j);
}
...
DatasetLayout DatasetLayout::fromTensorShapes(
    const std::vector<TensorShape> &tensors,
    ...
    for (const TensorShape &entry : tensors) {
        ...
        specs.push_back(TensorSpec{.name = entry.name,
    ...
    return DatasetLayout(offsetBytes, std::move(specs), std::move(windowedSpecs));
}
