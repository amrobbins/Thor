...
// Normalize each trailing component across the active tokens of each ragged row.
// The row partition is preserved exactly; inactive packed capacity is outside the
// logical normalization domain.
class SegmentedSoftmax : public MultiConnectionLayer {
   public:
    class Builder;

    SegmentedSoftmax() = default;
    ~SegmentedSoftmax() override = default;

    std::shared_ptr<Layer> clone() const override { return std::make_shared<SegmentedSoftmax>(*this); }
    std::string getLayerType() const override { return "SegmentedSoftmax"; }
...
class SegmentedSoftmax::Builder {
   public:
    virtual ~Builder() = default;
    virtual SegmentedSoftmax build();

    virtual Builder& network(Network& network) {
        if (_network.has_value()) throw std::runtime_error("SegmentedSoftmax network may only be set once.");
        _network = &network;
        return *this;
    }

    virtual Builder& featureInput(RaggedTensor featureInput) {
        if (_featureInput.has_value()) throw std::runtime_error("SegmentedSoftmax feature input may only be set once.");
        if (!featureInput.isInitialized()) throw std::invalid_argument("SegmentedSoftmax feature input must be initialized.");
