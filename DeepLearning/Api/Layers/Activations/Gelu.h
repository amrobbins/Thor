...
class Gelu : public Activation {
   public:
    ...
    std::shared_ptr<Layer> clone() const override {
        std::shared_ptr<Gelu> myClone = std::make_shared<Gelu>(*this);
    ...
    std::string getLayerType() const override { return "Gelu"; }

    static void deserialize(const nlohmann::json &j, Network *network) {
        if (j.at("version").get<std::string>() != "1.0.0")
            throw std::runtime_error("Unsupported version in Gelu::deserialize: " + j["version"].get<std::string>());
        if (j.at("layer_type").get<std::string>() != "gelu")
            throw std::runtime_error("Layer type mismatch in Gelu::deserialize: " + j.at("layer_type").get<std::string>());
   ...
   public:
    std::shared_ptr<Activation> build() override {
        ...
        if (_featureInput.has_value()) {
            ...
            applyStandaloneConfiguration(*gelu);
            gelu->initialized = true;
    ...
    Gelu::Builder &network(Network &_network) override {
        Activation::Builder::network(_network);
        return *this;
    }

    Gelu::Builder &featureInput(Tensor _featureInput) override {
        Activation::Builder::featureInput(_featureInput);
        return *this;
    }

    Gelu::Builder &featureInput(RaggedTensor _featureInput) override {
        Activation::Builder::featureInput(_featureInput);
        return *this;
    }
};
