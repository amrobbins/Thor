...
class HardSwish : public Activation {
   public:
    class Builder;
    HardSwish() {}
    ...
    std::shared_ptr<Layer> clone() const override {
        std::shared_ptr<HardSwish> myClone = std::make_shared<HardSwish>(*this);
    ...
    std::string getLayerType() const override { return "HardSwish"; }

    static void deserialize(const nlohmann::json &j, Network *network) {
        if (j.at("version").get<std::string>() != "1.0.0")
            throw std::runtime_error("Unsupported version in HardSwish::deserialize: " + j["version"].get<std::string>());
        if (j.at("layer_type").get<std::string>() != "hard_swish")
            throw std::runtime_error("Layer type mismatch in HardSwish::deserialize: " + j.at("layer_type").get<std::string>());
...
class HardSwish::Builder : public Activation::Builder {
   public:
    ...
    HardSwish::Builder &network(Network &_network) override {
