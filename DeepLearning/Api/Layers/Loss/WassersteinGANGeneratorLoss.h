...
class WassersteinGANGeneratorLoss : public Loss {
   public:
    class Builder;
    WassersteinGANGeneratorLoss() {}

    ~WassersteinGANGeneratorLoss() override {}

    std::shared_ptr<Layer> clone() const override { return std::make_shared<WassersteinGANGeneratorLoss>(*this); }

    std::string getLayerType() const override { return "WassersteinGANGeneratorLoss"; }
   ...
   protected:
    ...
    std::shared_ptr<ThorImplementation::Layer> stamp(ThorImplementation::TensorPlacement placement,
        ...
        (void)inferenceOnly;
        throw std::runtime_error("WassersteinGANGeneratorLoss is a compound API loss and should not be stamped directly.");
    }
...
class WassersteinGANGeneratorLoss::Builder {
   public:
    virtual ~Builder() = default;

    virtual WassersteinGANGeneratorLoss build() {
        ...
        wassersteinGANGeneratorLoss.buildSupportLayersAndAddToNetwork();

        return wassersteinGANGeneratorLoss;
    }

    virtual WassersteinGANGeneratorLoss::Builder &network(Network &_network) {
        THOR_THROW_IF_FALSE(!this->_network.has_value());
        this->_network = &_network;
        return *this;
    }

    virtual WassersteinGANGeneratorLoss::Builder &fakeScores(Tensor _fakeScores) {
    ...
    virtual WassersteinGANGeneratorLoss::Builder &reportsBatchLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::BATCH;
        return *this;
    }

    virtual WassersteinGANGeneratorLoss::Builder &reportsPerExampleLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::PER_EXAMPLE;
        return *this;
    }

    virtual WassersteinGANGeneratorLoss::Builder &reportsPerOutputLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::PER_OUTPUT;
        return *this;
    }

    virtual WassersteinGANGeneratorLoss::Builder &reportsNoLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::NONE;
        return *this;
    }

    virtual WassersteinGANGeneratorLoss::Builder &reportsRawLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
