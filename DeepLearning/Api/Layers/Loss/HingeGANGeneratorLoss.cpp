...
void validateScoreDType(const char* tensorName, DataType dtype) {
    if (dtype != DataType::FP16 && dtype != DataType::FP32) {
        throw runtime_error(string("Unsupported HingeGANGeneratorLoss ") + tensorName + " dtype: " +
                            ThorImplementation::TensorDescriptor::getElementTypeName(dtype));
    }
}

ThorImplementation::DynamicExpression makeHingeGANGeneratorLossExpression(DataType lossDataType) {
...
ThorImplementation::DynamicExpression makeHingeGANGeneratorGradientExpression(DataType scoresDataType) {
...
void HingeGANGeneratorLoss::buildSupportLayersAndAddToNetwork() {
    ...
    MultiInputCustomLoss rawHingeGANGeneratorLoss = MultiInputCustomLoss::Builder()
                                                        .network(*network)
                                                        .lossExpression(makeHingeGANGeneratorLossExpression(lossDataType))
                                                        .gradientExpression(
                                                            makeHingeGANGeneratorGradientExpression(fakeScoresTensor.getDataType()))
    ...
    lossShaperInput = rawHingeGANGeneratorLoss.getLoss();
...
json HingeGANGeneratorLoss::architectureJson() const {
    json j;
    j["factory"] = Factory::Loss.value();
    j["version"] = getLayerVersion();
    j["layer_type"] = "hinge_gan_generator_loss";
...
void HingeGANGeneratorLoss::deserialize(const json& j, Network* network) {
    if (j.at("version").get<std::string>() != "1.0.0")
        throw runtime_error("Unsupported version in HingeGANGeneratorLoss::deserialize: " + j["version"].get<std::string>());
    if (j.at("layer_type").get<std::string>() != "hinge_gan_generator_loss")
        throw runtime_error("Layer type mismatch in HingeGANGeneratorLoss::deserialize: " + j.at("layer_type").get<std::string>());
    ...
    hingeGANGeneratorLoss.lossShape = j.at("loss_shape").get<LossShape>();
    hingeGANGeneratorLoss.lossDataType = j.at("loss_data_type").get<DataType>();

    hingeGANGeneratorLoss.lossWeight = ThorImplementation::lossWeightFromJson(j);
    hingeGANGeneratorLoss.fakeScoresTensor = fakeScores;
    hingeGANGeneratorLoss.network = network;
...
static bool registered = []() {
    Thor::Loss::register_layer("hinge_gan_generator_loss", &Thor::HingeGANGeneratorLoss::deserialize);
    return true;
}();
}  // namespace
