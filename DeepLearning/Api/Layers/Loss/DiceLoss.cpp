...
void validateLabelsDType(DataType dtype) {
    switch (dtype) {
        ...
        case DataType::FP32:
            return;
        default:
            throw runtime_error("Unsupported DiceLoss label dtype: " + ThorImplementation::TensorDescriptor::getElementTypeName(dtype));
    }
}

void validatePredictionsDType(DataType dtype) {
    if (dtype != DataType::FP16 && dtype != DataType::FP32) {
        throw runtime_error("Unsupported DiceLoss predictions dtype: " + ThorImplementation::TensorDescriptor::getElementTypeName(dtype));
    }
}
...
ThorImplementation::DynamicExpression makeDiceLossExpression(DataType lossDataType,
                                                             float smooth,
                                                             const std::vector<uint64_t>& spatialReductionAxes,
    ...
    ThorImplementation::Expression intersection = (predictions * labels).reduce_sum(spatialReductionAxes, lossSqueezeAxes, DataType::FP32);
    ThorImplementation::Expression predictionSum = predictions.reduce_sum(spatialReductionAxes, lossSqueezeAxes, DataType::FP32);
    ThorImplementation::Expression labelSum = labels.reduce_sum(spatialReductionAxes, lossSqueezeAxes, DataType::FP32);
    ThorImplementation::Expression numerator = (two * intersection) + smoothExpr;
    ThorImplementation::Expression denominator = (predictionSum + labelSum + smoothExpr).max(ThorImplementation::Expression(kOverlapEpsilon));
    ThorImplementation::Expression loss = (one - (numerator / denominator)).withOutputDType(lossDataType);
...
ThorImplementation::DynamicExpression makeDiceGradientExpression(DataType predictionsDataType,
                                                                 float smooth,
                                                                 const std::vector<uint64_t>& spatialReductionAxes) {
    ...
    ThorImplementation::Expression intersection = (predictions * labels).reduce_sum(spatialReductionAxes, {}, DataType::FP32);
    ThorImplementation::Expression predictionSum = predictions.reduce_sum(spatialReductionAxes, {}, DataType::FP32);
    ThorImplementation::Expression labelSum = labels.reduce_sum(spatialReductionAxes, {}, DataType::FP32);
    ThorImplementation::Expression numerator = (two * intersection) + smoothExpr;
    ThorImplementation::Expression denominator = (predictionSum + labelSum + smoothExpr).max(ThorImplementation::Expression(kOverlapEpsilon));
    ThorImplementation::Expression gradient =
        (((numerator - (two * labels * denominator)) / (denominator * denominator)) *
...
void DiceLoss::buildSupportLayersAndAddToNetwork() {
    ...
    CustomLoss rawLoss = CustomLoss::Builder()
                             .network(*network)
                             .lossExpression(makeDiceLossExpression(lossDataType, smooth, spatialReductionAxes, lossSqueezeAxes))
                             .gradientExpression(makeDiceGradientExpression(predictionsTensor.getDataType(), smooth, spatialReductionAxes))
...
json DiceLoss::architectureJson() const {
...
void DiceLoss::deserialize(const json& j, Network* network) {
    if (j.at("version").get<std::string>() != "1.0.0")
        throw runtime_error("Unsupported version in DiceLoss::deserialize: " + j["version"].get<std::string>());
    if (j.at("layer_type").get<std::string>() != "dice_loss")
        throw runtime_error("Layer type mismatch in DiceLoss::deserialize: " + j.at("layer_type").get<std::string>());
...
static bool registered = []() {
    Thor::Loss::register_layer("dice_loss", &Thor::DiceLoss::deserialize);
    return true;
}();
}  // namespace
