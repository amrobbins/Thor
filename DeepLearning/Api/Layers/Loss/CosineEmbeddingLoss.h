...
class CosineEmbeddingLoss : public Loss {
   public:
    class Builder;
    CosineEmbeddingLoss() {}

    ~CosineEmbeddingLoss() override {}

    std::shared_ptr<Layer> clone() const override { return std::make_shared<CosineEmbeddingLoss>(*this); }

    std::string getLayerType() const override { return "CosineEmbeddingLoss"; }
   ...
   protected:
    ...
    std::shared_ptr<ThorImplementation::Layer> stamp(ThorImplementation::TensorPlacement placement,
        ...
        (void)inferenceOnly;
        throw std::runtime_error("CosineEmbeddingLoss is a compound API loss and should not be stamped directly.");
    }
...
class CosineEmbeddingLoss::Builder {
   public:
    virtual ~Builder() = default;

    virtual CosineEmbeddingLoss build() {
        ...
        THOR_THROW_IF_FALSE(_input1.value().getDimensions().size() == 1);
        THOR_THROW_IF_FALSE(_input1.value().getDimensions()[0] > 0);
        THOR_THROW_IF_FALSE(_input1.value().getDimensions() == _input2.value().getDimensions());
        THOR_THROW_IF_FALSE(_target.value().getDimensions().size() == 1);
        THOR_THROW_IF_FALSE(_target.value().getDimensions()[0] == 1);
        ...
        cosineEmbeddingLoss.input1Tensor = _input1.value();
        ...
        cosineEmbeddingLoss.network = _network.value();
    ...
    virtual CosineEmbeddingLoss::Builder &network(Network &_network) {
        THOR_THROW_IF_FALSE(!this->_network.has_value());
        this->_network = &_network;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &input1(Tensor _input1) {
    ...
    virtual CosineEmbeddingLoss::Builder &input2(Tensor _input2) {
    ...
    virtual CosineEmbeddingLoss::Builder &target(Tensor _target) {
    ...
    virtual CosineEmbeddingLoss::Builder &margin(float _margin) {
        THOR_THROW_IF_FALSE(!this->_margin.has_value());
        THOR_THROW_IF_FALSE(_margin >= -1.0f && _margin <= 1.0f);
        this->_margin = _margin;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &eps(float _eps) {
    ...
    virtual CosineEmbeddingLoss::Builder &reportsBatchLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::BATCH;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &reportsPerExampleLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::PER_EXAMPLE;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &reportsPerOutputLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::PER_OUTPUT;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &reportsNoLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
        _lossShape = LossShape::NONE;
        return *this;
    }

    virtual CosineEmbeddingLoss::Builder &reportsRawLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
