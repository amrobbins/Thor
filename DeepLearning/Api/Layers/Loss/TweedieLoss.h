...
class TweedieLoss : public Loss {
   public:
    class Builder;
    TweedieLoss() {}

    ~TweedieLoss() override {}

    std::shared_ptr<Layer> clone() const override { return std::make_shared<TweedieLoss>(*this); }

    std::string getLayerType() const override { return "TweedieLoss"; }

    float getPower() const { return power; }
    float getEps() const { return eps; }
    [[nodiscard]] bool isRagged() const { return raggedPredictionsTensor.has_value(); }
    [[nodiscard]] RaggedTensor getRaggedPredictions() const { if (!isRagged()) throw std::runtime_error("TweedieLoss predictions are dense."); return raggedPredictionsTensor.value(); }
    [[nodiscard]] RaggedTensor getRaggedLabels() const { if (!isRagged()) throw std::runtime_error("TweedieLoss labels are dense."); return raggedLabelsTensor.value(); }
    [[nodiscard]] RaggedTensor getRaggedRawLoss() const { if (!raggedRawLossTensor.has_value()) throw std::runtime_error("TweedieLoss raw loss is dense."); return raggedRawLossTensor.value(); }
    [[nodiscard]] RaggedTensor getRaggedLoss() const { if (!isRagged() || lossShape != LossShape::RAW || !raggedRawLossTensor.has_value()) throw std::runtime_error("TweedieLoss does not expose a ragged reported loss for this LossShape."); return raggedRawLossTensor.value(); }
   ...
   protected:
    ...
    std::shared_ptr<ThorImplementation::Layer> stamp(ThorImplementation::TensorPlacement placement,
        ...
        (void)inferenceOnly;
        throw std::runtime_error("TweedieLoss is a compound API loss and should not be stamped directly.");
    }
...
class TweedieLoss::Builder {
   public:
    virtual ~Builder() = default;

    virtual TweedieLoss build() {
        ...
        const float power = _power.value_or(1.5f);
        ...
        if (hasDensePredictions) {
            ...
            if (_exampleWeights.has_value() && _exampleWeights->getDimensions() != std::vector<uint64_t>{1})
                throw std::invalid_argument("TweedieLoss ragged example_weights must have dimensions [1] for one scalar weight per logical row.");
            if (_lossShape.value() == LossShape::PER_OUTPUT) throw std::invalid_argument("TweedieLoss LossShape::PER_OUTPUT is undefined for ragged sequences.");
    ...
    virtual TweedieLoss::Builder &network(Network &_network) {
        THOR_THROW_IF_FALSE(!this->_network.has_value());
        this->_network = &_network;
        return *this;
    }

    virtual TweedieLoss::Builder &predictions(Tensor _predictions) {
    ...
    virtual TweedieLoss::Builder &predictions(RaggedTensor predictions) { THOR_THROW_IF_FALSE(!this->_raggedPredictions.has_value()); THOR_THROW_IF_FALSE(predictions.isInitialized()); this->_raggedPredictions = std::move(predictions); return *this; }
    virtual TweedieLoss::Builder &mean(Tensor _mean) { return predictions(_mean); }
    virtual TweedieLoss::Builder &mean(RaggedTensor mean) { return predictions(std::move(mean)); }

    virtual TweedieLoss::Builder &labels(Tensor _labels) {
    ...
    virtual TweedieLoss::Builder &labels(RaggedTensor labels) { THOR_THROW_IF_FALSE(!this->_raggedLabels.has_value()); THOR_THROW_IF_FALSE(labels.isInitialized()); this->_raggedLabels = std::move(labels); return *this; }
    virtual TweedieLoss::Builder &target(Tensor _target) { return labels(_target); }
    virtual TweedieLoss::Builder &target(RaggedTensor target) { return labels(std::move(target)); }
    ...
    virtual TweedieLoss::Builder &power(float _power) {
        ...
        THOR_THROW_IF_FALSE(std::isfinite(_power));
        this->_power = _power;
        return *this;
    }

    virtual TweedieLoss::Builder &eps(float _eps) {
    ...
    virtual TweedieLoss::Builder &reportsBatchLoss() {
    ...
    virtual TweedieLoss::Builder &reportsRawLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
