...
void validateLabelsDType(DataType dtype) {
    switch (dtype) {
        ...
        case DataType::FP32:
            return;
        default:
            throw runtime_error("Unsupported FocalTverskyLoss label dtype: " + ThorImplementation::TensorDescriptor::getElementTypeName(dtype));
    }
}

void validatePredictionsDType(DataType dtype) {
    if (dtype != DataType::FP16 && dtype != DataType::FP32) {
        throw runtime_error("Unsupported FocalTverskyLoss predictions dtype: " + ThorImplementation::TensorDescriptor::getElementTypeName(dtype));
    }
}
...
ThorImplementation::DynamicExpression makeFocalTverskyLossExpression(DataType lossDataType,
                                                                     float alpha,
                                                                     float beta,
                                                                     float gamma,
                                                                     float smooth,
    ...
    ThorImplementation::Expression tversky = terms.numerator / terms.denominator;
    ThorImplementation::Expression oneMinusTversky = (one - tversky).max(ThorImplementation::Expression(kOverlapEpsilon));
    ThorImplementation::Expression loss = oneMinusTversky.pow(ThorImplementation::Expression(gamma)).withOutputDType(lossDataType);
...
ThorImplementation::DynamicExpression makeFocalTverskyGradientExpression(DataType predictionsDataType,
                                                                         float alpha,
                                                                         float beta,
                                                                         float gamma,
                                                                         float smooth,
    ...
    ThorImplementation::Expression tversky = terms.numerator / terms.denominator;
    ThorImplementation::Expression oneMinusTversky = (one - tversky).max(ThorImplementation::Expression(kOverlapEpsilon));
    ThorImplementation::Expression tverskyLossGradient =
    ...
    ThorImplementation::Expression gradient =
        (ThorImplementation::Expression(gamma) * oneMinusTversky.pow(ThorImplementation::Expression(gamma - 1.0f)) *
         tverskyLossGradient * ThorImplementation::Expression(ThorImplementation::Loss::getLossScalingFactor()))
...
void FocalTverskyLoss::buildSupportLayersAndAddToNetwork() {
    ...
    CustomLoss rawLoss = CustomLoss::Builder()
                             .network(*network)
                             .lossExpression(makeFocalTverskyLossExpression(lossDataType, alpha, beta, gamma, smooth, spatialReductionAxes, lossSqueezeAxes))
                             .gradientExpression(makeFocalTverskyGradientExpression(predictionsTensor.getDataType(), alpha, beta, gamma, smooth, spatialReductionAxes))
...
json FocalTverskyLoss::architectureJson() const {
...
void FocalTverskyLoss::deserialize(const json& j, Network* network) {
    if (j.at("version").get<std::string>() != "1.0.0")
        throw runtime_error("Unsupported version in FocalTverskyLoss::deserialize: " + j["version"].get<std::string>());
    if (j.at("layer_type").get<std::string>() != "focal_tversky_loss")
        throw runtime_error("Layer type mismatch in FocalTverskyLoss::deserialize: " + j.at("layer_type").get<std::string>());
...
static bool registered = []() {
    Thor::Loss::register_layer("focal_tversky_loss", &Thor::FocalTverskyLoss::deserialize);
    return true;
}();
}  // namespace
