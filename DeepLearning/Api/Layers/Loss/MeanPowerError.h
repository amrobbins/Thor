...
class MeanPowerError::Builder {
   public:
    ...
    virtual MeanPowerError build() {
        ...
        if (hasDensePredictions) {
            ...
            if (!predictions.sharesPartitionWith(labels))
                throw std::invalid_argument("MeanPowerError ragged predictions and labels must use the exact same row partition.");
    ...
    virtual MeanPowerError::Builder &reportsPerExampleLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
    ...
    virtual MeanPowerError::Builder &reportsNoLoss() {
        THOR_THROW_IF_FALSE(!this->_lossShape.has_value());
