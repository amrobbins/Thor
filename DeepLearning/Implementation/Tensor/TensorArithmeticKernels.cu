...
void Tensor::launchGpuFillRandom(void *mem, uint64_t numElements, double minValue, double maxValue, Stream stream) {
    ...
    if constexpr (is_same<DATA_TYPE, half>::value) {
        ...
        setRandomValues<__nv_bfloat16, float>
            <<<gridSize, blockSize, 0, stream>>>((__nv_bfloat16 *)mem, numElements, minValue, range, seed);
    } else if constexpr (is_same<DATA_TYPE, __nv_fp8_e4m3>::value) {
        setRandomValues<__nv_fp8_e4m3, float>
            <<<gridSize, blockSize, 0, stream>>>((__nv_fp8_e4m3 *)mem, numElements, minValue, range, seed);
    } else if constexpr (is_same<DATA_TYPE, __nv_fp8_e5m2>::value) {
        setRandomValues<__nv_fp8_e5m2, float>
            <<<gridSize, blockSize, 0, stream>>>((__nv_fp8_e5m2 *)mem, numElements, minValue, range, seed);
