...
class RaggedToPaddedDense : public MultiConnectionLayer {
   public:
    ...
    uint64_t logicalByteCountForward(uint64_t validExampleCount) override {
        ...
        return logicalAdapterBytes(featureInputs[0].value(), inputDescriptor.getMaxTotalValues(),
                                   featureOutputs[0].value(), denseLogicalValueCapacity(), *activeValues,
                                   "RaggedToPaddedDense forward");
    }
    uint64_t logicalByteCountBackward(uint64_t validExampleCount) override {
        ...
        return logicalAdapterBytes(errorInputs[0].value(), denseLogicalValueCapacity(),
                                   errorOutputs[0].value(), inputDescriptor.getMaxTotalValues(), *activeValues,
                                   "RaggedToPaddedDense backward");
    }
   ...
   private:
    [[nodiscard]] std::optional<uint64_t> logicalActiveValues(uint64_t validExampleCount) const noexcept {
        try {
            ...
            if (validExampleCount == 0) {
                ...
                if (!active.has_value() || *active > inputDescriptor.getMaxTotalValues()) return std::nullopt;
                return active;
            }
            if (validExampleCount > inputDescriptor.getBatchSize()) return std::nullopt;
    ...
    [[nodiscard]] uint64_t denseLogicalValueCapacity() const noexcept {
        try {
            const std::vector<uint64_t> dims = outputDescriptor.getDimensions();
            if (dims.size() < 2 || dims[0] != inputDescriptor.getBatchSize()) return 0;
