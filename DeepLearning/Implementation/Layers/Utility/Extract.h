...
class Extract : public Layer {
   public:
    ...
    Extract(vector<pair<unsigned int, unsigned int>> dimensionSpans) {
    ...
    virtual Optional<Tensor> createFeatureOutputTensor() {
        ...
        vector<unsigned long> outputTensorDimensions = getExtractedTensorDimensions(dimensionSpans);
    ...
    virtual void compile() {
        ...
        for (unsigned int d = 0; d < dimensionSpans.size(); ++d)
            paddingAmount[d] =
                pair<unsigned int, unsigned int>(dimensionSpans[d].first, (inputTensorDimensions[d] - 1) - dimensionSpans[d].second);
    ...
    vector<unsigned long> getExtractedTensorDimensions(vector<pair<unsigned int, unsigned int>> dimensionSpans) {
