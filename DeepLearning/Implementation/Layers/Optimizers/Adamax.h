...
class Adamax final : public CustomOptimizer {
   public:
    Adamax(uint64_t id, float alpha, float beta1, float beta2, float epsilon);
   ...
   private:
    Adamax(uint64_t id, std::shared_ptr<RuntimeState> runtimeState);
