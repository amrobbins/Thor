   ...
   public:
    ...
    virtual void computeMetric(Tensor labels, Tensor predictions, Tensor metric, Stream stream) {
        if (labelFormat == LABEL_FORMAT::INDICATOR_PER_CLASS_TYPE) {
            ...
            float *labelArray = (float *)labels_h.getMemPtr();
            ...
            vector<float> bestLabelVector;
            for (uint32_t b = 0; b < 6; ++b) {
                float bestLabel = labelArray[b * numClasses];
                ...
                for (uint32_t c = 1; c < numClasses; ++c) {
                    float classLabel = labelArray[b * numClasses + c];
            ...
            for (uint32_t i = 0; i < labelVector.size(); ++i)
                printf("%d(%.2f) ", labelVector[i], bestLabelVector[i]);
            printf("\n");
            printf("\rPredictions: ");
