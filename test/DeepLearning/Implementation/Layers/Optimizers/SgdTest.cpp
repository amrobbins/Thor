...
static void verifyMatricesMatch(
...
//
// FIXME: This does not seem to pass, consistently failing at the last verifyMatricesMatch.
//        It's unknown whether this is a bug with Nesterov momentum or with the test case or both.
//
...
// 4. call forward
// 5. check inference is correct: this time there is no previous weight update to momentum so update with just gradient
// 6. call backward
...
// 13. Call forward for a third time, this time in inference mode, to ensure that projected weights are not used to create featureOutput
// 14. Verify featureOutput
TEST(SgdTest, DISABLED_TestWeightsUpdateWithNesterovMomentum) {
    srand(time(nullptr));
    ...
    for (uint32_t t = 0; t < 50; ++t) {
        ...
        matrixMultiplyCpuHalf(featureInMem_h,
                              ...
                              false,
                              false);
        verifyMatricesMatch(featureOutMem_h, featureOutGpuMem_h, batchSize, numOutputFeatures, true, numInputFeatures * 0.03, 0.03f);
    }
}
