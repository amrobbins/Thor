...
//     half *expected, half *actual, uint32_t rows, uint32_t cols, bool print = false, float staticThresh = 0.1, float dynamicThresh =
//     0.004) { double meanAbsoluteError = 0.0; bool foundError = false; for (uint32_t row = 0; row < rows; ++row) {
...
//
//     layers.push_back(make_shared<NetworkInput>(gpuPlacement, TensorDescriptor::DataType::FP16,
//     featureIn.getDescriptor().getDimensions())); layers.push_back(make_shared<NoOpLayer>()); shared_ptr<FullyConnected>
//     fullyConnectedLayer = make_shared<FullyConnected>(numOutputFeatures, hasBias);
...
//     layers.push_back(std::make_shared<NetworkInput>(gpuPlacement, TensorDescriptor::DataType::FP16, std::vector<uint64_t>{batchSize,
//     inF})); layers.push_back(std::make_shared<NoOpLayer>());
//
...
//     std::vector<std::shared_ptr<Layer>> layers;
//     layers.push_back(
//         std::make_shared<NetworkInput>(gpuPlacement, TensorDescriptor::DataType::FP16, std::vector<uint64_t>{batchSize,
//         numInputFeatures}));
//     layers.push_back(std::make_shared<NoOpLayer>());
//
...
//     std::vector<std::shared_ptr<Layer>> layers;
//     layers.push_back(std::make_shared<NetworkInput>(gpuPlacement, TensorDescriptor::DataType::FP16, std::vector<uint64_t>{batchSize,
//     inF})); layers.push_back(std::make_shared<NoOpLayer>()); auto fc = std::make_shared<FullyConnected>(outF, hasBias);
//     layers.push_back(fc);
//     layers.push_back(std::make_shared<NoOpLayer>());
//     layers.push_back(std::make_shared<NetworkOutput>(cpuPlacement));
//     LayerTestHelper::connectNetwork(layers);
