...
TEST(MeanAbsolutePercentageError, ComputesCorrectResult_FP16) {
    ...
    for (uint32_t test = 0; test < 10; ++test) {
        ...
        ASSERT_TRUE(!meanAbsolutePercentageError->getErrorInput().has_value());
        if (inferenceOnly) {
            ASSERT_TRUE(!meanAbsolutePercentageError->getErrorOutput().has_value());
        }
        ...
        if (!inferenceOnly) {
            ...
            elementLossGradientGpu_h.copyFromAsync(meanAbsolutePercentageError->getErrorOutput().value(), labelsStream);
            labelsStream.synchronize();
