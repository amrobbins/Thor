...
TEST(Activations, SoftSignBuilds) {
    ...
    std::optional<Tensor> actualInput = softSign->getFeatureInput();
    ...
    std::optional<Tensor> actualOutput = softSign->getFeatureOutput();
...
TEST(Activations, SoftSignSerializeDeserialize) {
    ...
    SoftSign::Builder softSignBuilder = SoftSign::Builder().network(initialNetwork).featureInput(networkInput.getFeatureOutput().value());
    shared_ptr<SoftSign> softSign = dynamic_pointer_cast<SoftSign>(softSignBuilder.build());

    NetworkOutput networkOutput = NetworkOutput::Builder()
                                      .network(initialNetwork)
                                      .name("testOutput")
                                      .inputTensor(softSign->getFeatureOutput().value())
                                      .dataType(dataType)
                                      .build();

    ASSERT_TRUE(softSign->isInitialized());

    Tensor featureInput = softSign->getFeatureInput().value();
    Tensor featureOutput = softSign->getFeatureOutput().value();
    assert(featureInput == networkInput.getFeatureOutput());

    ASSERT_TRUE(softSign->getFeatureOutput().has_value());
    ASSERT_EQ(softSign->getFeatureOutput().value(), featureOutput);

    ASSERT_TRUE(softSign->getFeatureInput().has_value());
    assert(softSign->getFeatureInput().value() == featureInput);
    ...
    json softSignJ = softSign->serialize(archiveWriter, stream);
    ...
    shared_ptr<ThorImplementation::CustomLayer> stampedSoftSign = dynamic_pointer_cast<ThorImplementation::CustomLayer>(newStamp.getTrainableLayer(0));
    ASSERT_NE(stampedSoftSign, nullptr);
    ASSERT_EQ(stampedSoftSign->getLayerType(), "CustomLayer<SoftSign>");
    ...
    ASSERT_TRUE(stampedSoftSign->getFeatureOutput().has_value());
    ASSERT_TRUE(stampedOutput->getFeatureOutput().has_value());
    ASSERT_EQ(stampedInput->getFeatureOutput().value(), stampedSoftSign->getFeatureInput().value());
    ASSERT_EQ(stampedSoftSign->getFeatureOutput().value(), stampedOutput->getFeatureInput().value());
...
TEST(Activations, SoftSignRegistered) {
    ...
    shared_ptr<ThorImplementation::CustomLayer> stampedSoftSign = dynamic_pointer_cast<ThorImplementation::CustomLayer>(stampedNetwork.getTrainableLayer(0));
    ASSERT_NE(stampedSoftSign, nullptr);
    ASSERT_EQ(stampedSoftSign->getLayerType(), "CustomLayer<SoftSign>");
