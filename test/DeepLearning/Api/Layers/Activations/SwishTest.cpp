...
TEST(Activations, SwishSerializeDeserialize) {
    ...
    json swishJ = swish->serialize(archiveWriter, stream);
    ...
    shared_ptr<ThorImplementation::CustomLayer> stampedSwish = dynamic_pointer_cast<ThorImplementation::CustomLayer>(newStamp.getTrainableLayer(0));
    ASSERT_NE(stampedSwish, nullptr);
    ASSERT_EQ(stampedSwish->getLayerType(), "CustomLayer<Swish>");
...
TEST(Activations, SwishRegistered) {
    ...
    shared_ptr<ThorImplementation::CustomLayer> stampedSwish = dynamic_pointer_cast<ThorImplementation::CustomLayer>(stampedNetwork.getTrainableLayer(0));
    ASSERT_NE(stampedSwish, nullptr);
    ASSERT_EQ(stampedSwish->getLayerType(), "CustomLayer<Swish>");
