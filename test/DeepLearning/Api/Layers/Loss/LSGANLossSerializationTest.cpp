...
struct LSGANLossNetworkParts {
    LSGANDiscriminatorLoss discriminatorLoss;
    LSGANGeneratorLoss generatorLoss;
};

LSGANLossNetworkParts addLSGANLosses(Network& network) {
    ...
    LSGANDiscriminatorLoss discriminatorLoss = LSGANDiscriminatorLoss::Builder()
                                                       .network(network)
                                                       ...
                                                       .build();
    consumeLoss(network, "lsgan_discriminator_loss", discriminatorLoss.getLoss());

    NetworkInput fakeScoresForGenerator = fp32Input(network, "fake_scores_g", {3});
    LSGANGeneratorLoss generatorLoss = LSGANGeneratorLoss::Builder()
                                             .network(network)
                                             ...
                                             .lossDataType(DataType::FP32)
                                             .build();
    consumeLoss(network, "lsgan_generator_loss", generatorLoss.getLoss());

    return LSGANLossNetworkParts{discriminatorLoss, generatorLoss};
}
...
TEST(LSGANLossSerialization, PublicArchitectureJsonIncludesRoundTripFields) {
    Network network("lsgan_loss_public_json");
    LSGANLossNetworkParts losses = addLSGANLosses(network);
    ...
    EXPECT_EQ(discriminatorJson.at("layer_type").get<string>(), "lsgan_discriminator_loss");
    EXPECT_EQ(discriminatorJson.at("loss_shape").get<Loss::LossShape>(), Loss::LossShape::PER_EXAMPLE);
    EXPECT_EQ(discriminatorJson.at("loss_data_type").get<DataType>(), DataType::FP32);
    EXPECT_FLOAT_EQ(discriminatorJson.at("real_target").get<float>(), 0.9f);
    EXPECT_FLOAT_EQ(discriminatorJson.at("fake_target").get<float>(), -0.1f);
    ...
    EXPECT_EQ(generatorJson.at("layer_type").get<string>(), "lsgan_generator_loss");
    ...
    EXPECT_FLOAT_EQ(generatorJson.at("target").get<float>(), 0.8f);
...
TEST(LSGANLossSerialization, PublicLossJsonDeserializersRebuildSupportLayers) {
    Network originalNetwork("lsgan_loss_public_deserialize_source");
    LSGANLossNetworkParts losses = addLSGANLosses(originalNetwork);

    const json originalArchitecture = originalNetwork.architectureJson();
    Network restoredNetwork("lsgan_loss_public_deserialize_restored");
...
TEST(LSGANLossSerialization, NetworkSaveLoadRoundTripRestoresSupportLayers) {
    const string networkName = "lsgan_loss_save_load";
    filesystem::path archiveDir = makeUniqueTestArchiveDir(networkName);

    try {
        Network network(networkName);
        addLSGANLosses(network);
...
TEST(LSGANLossSerialization, LoadedMultiInputCustomLossPreservesDifferentiableInputs) {
    const string networkName = "lsgan_loss_multi_input_round_trip";
    filesystem::path archiveDir = makeUniqueTestArchiveDir(networkName);
