...
TEST(MeanAbsolutePercentageError, SerializeDeserialize) {
    ...
    if (stampedLossShaper) {
        ASSERT_EQ(stampedMeanAbsolutePercentageError->getLossOutput().get(), stampedLossShaper->getFeatureInput().get());
        ASSERT_EQ(stampedLossShaper->getFeatureOutput().get(), stampedLossOutput->getFeatureInput().get());
    } else if (stampedReshape) {
        ASSERT_EQ(stampedMeanAbsolutePercentageError->getLossOutput().get(), stampedReshape->getFeatureInput().get());
