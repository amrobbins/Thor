...
TEST(CublasMatrixMultiply, ChooseOptimalKernelWorksFP32) {
    ...
    for (int i = 0; i < 1; ++i) {
        int rowsA = 2;  // 128 + (rand() % 1024);
        int colsA = 2;  // 128 + (rand() % 1024);
        int colsB = 2;  // 128 + (rand() % 1024);
        ...
        printf("\n\nA:\n");
        printMatrix(AMem, rowsA, colsA, ldC);
        printf("\n\nB:\n");
        printMatrix(BMem, colsA, colsB, ldC);
        printf("\n\nC before:\n");
