...
TEST(CudnnWorkspaceConcurrencyStress, SharedRmsNormAndAttentionPlansUseIndependentScratchAcrossBranches) {
    ...
    const uint64_t rmsForwardBytes = rmsNorm.forwardWorkspaceSizeInBytes(rmsDescriptor, GPU_NUM);
    const uint64_t rmsBackwardBytes = rmsNorm.backwardWorkspaceSizeInBytes(rmsDescriptor, GPU_NUM);
    ASSERT_EQ(rmsNorm.cachedGraphCount(), 2U);
    ...
    for (int iteration = 0; iteration < STRESS_ITERATIONS; ++iteration) {
        for (size_t i = 0; i < BRANCH_COUNT; ++i) {
            ...
            rmsNorm.forward(rmsDescriptor, rmsForward, rmsBranches[i].forwardWorkspace, *streams[i]);

            CudnnAttentionForwardArgs attentionForward = attentionForwardArgs(attentionBranches[i]);
            attention.forward(attentionDescriptor, attentionForward, attentionBranches[i].forwardWorkspace, *streams[i]);

            CudnnRmsNormBackwardArgs rmsBackward = rmsBackwardArgs(rmsBranches[i]);
            rmsNorm.backward(rmsDescriptor, rmsBackward, rmsBranches[i].backwardWorkspace, *streams[i]);

            CudnnAttentionBackwardArgs attentionBackward = attentionBackwardArgs(attentionBranches[i]);
            attention.backward(attentionDescriptor, attentionBackward, attentionBranches[i].backwardWorkspace, *streams[i]);
        }
    }
    ...
    // Repeated execution must continue to use the same two plans per wrapper.
    EXPECT_EQ(rmsNorm.cachedGraphCount(), 2U);
