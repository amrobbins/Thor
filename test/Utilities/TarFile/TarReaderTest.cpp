...
static FooterInfo read_footer_info(const fs::path& shard_path) {
...
static void corrupt_archive_id_in_shard(const fs::path& shard_path) {
    FooterInfo fi = read_footer_info(shard_path);
...
TEST(TarRoundTrip, RejectsArchiveIdMismatchAcrossThreeShards) {
    ...
    // Force 3 shards deterministically:
    // - Use a shard limit small enough that after writing one big file (allowed even if > limit),
    //   the next add will rollover because s.pos > 0 and s.pos + est > limit.
    const uint64_t shard_limit = 256 * 1024;  // 256 KiB
    const bool overwrite = true;

    // Each file is 512 KiB so we get one per shard (3 files => 3 shards).
    ...
    ASSERT_NO_THROW(corrupt_archive_id_in_shard(s1));
    ...
    try {
        ...
        // Don't overfit to exact wording, but this makes sure we're failing for the expected reason.
